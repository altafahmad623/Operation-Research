...
threePoint circle(double **c, int m, int n, double **x, coordinates theta, bool isSelect[100][100]) // this function helps to find the loop inside the table to update the table in MODi method
{
    // look to the right
    ...
    {
        if (isSelect[theta.x][j]) //If there is a cell with value on this side then it looks for cells above nad below it which should have value, that is it is allocated or not
        {
            for (int i = 1; i < (m - theta.x); i++) // this looks for cell above the theta cell
            {
                if ((isSelect[theta.x + i][j]) && (isSelect[theta.x + i][theta.y])) // and if found it returns it
                {
            ...
            {
                if ((isSelect[theta.x - i][j]) && (isSelect[theta.x - i][theta.y]))
                {
                    point.neg1.x = theta.x;
                    point.neg1.y = j;
                    point.neg2.x = theta.x - i;
    ...
    {
        if (isSelect[i][theta.y])
        {
            for (int j = 1; j < (n - theta.y); j++)
            {
                if (isSelect[i][theta.y + j] && isSelect[theta.x][theta.y + j])
                {
                    point.neg1.x = theta.x;
...
int MODIoptim(double *a, double *b, double **c, int m, int n, double **x) // Finds the optimum solution after the basic solution is found out by the North west corner or least cost cell method depending on the problem
{
    ...
    if (count != (m + n - 1))
    {
        cout << "MO DI method cannot be done on this. We have to remove the degneracy \n";
        int pq = 0;
        for (int i = 0; i < m; i++)
        {
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            pts = circle(c, m, n, x3, p, isSelect); // finds the three points in the loop
