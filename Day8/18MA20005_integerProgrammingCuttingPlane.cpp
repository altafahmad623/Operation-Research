...
int gomoryCuttingPlane(double **A, double *B, double *C, int n, int m, int msum, int *sign, int maxmin) // the function which solves the LPP using simpmlex method
{
    double sum;
    int i, j, k = n + msum;
    int num_equations = m;
    int num_var = n + msum;
    int doublecheck = 0;
    // now we need to generate the gomoryCuttingPlane table first and foremost
    ...
    while (check)
    {
        ...
        solkey = sol[keyrow];
        //now we print the gomoryCuttingPlane table for each iteration
        printf("\n\nIteration no: %d\n", iter);
        printf("\n\t CB_i \t C_j ");
        for (i = 0; i < (n + msum); i++)
        {
...
int main() // menu driven program
{
    ...
    ck = gomoryCuttingPlane(A, B, C, n, m, msum, sign, maxmin);
    //printf("%d",msum);
    return 0;
}
