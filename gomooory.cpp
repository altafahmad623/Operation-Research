...
int gomory(double **A, double *B, double *C, int n, int m, int msum, int *sign, int maxmin) // the function which solves the LPP using simpmlex method
{
    double sum;
    int i, j, k = n + msum;
    int num_equations = m;
    ...
    while (check)
    {
        ...
        solkey = sol[keyrow];
        //now we print the gomory table for each iteration
        printf("\n\nIteration no: %d\n", iter);
        printf("\n\t CB_i \t C_j ");
        for (i = 0; i < (n + msum); i++)
        {
    ...
    while (1)
    {
        ...
        for (i = 0; i < (num_var); i++)
        {
            ...
            {
                sim[maxfrac_ind][i] += epsilon;
                fractPart = modf(sim[maxfrac_ind][i], &intPart);
                fractPart -= epsilon;
                sim[maxfrac_ind][i] -= epsilon;
                if (fractPart > 0.0)
                {
        ...
        for (i = 0; i < num_equations; i++)
        {
            ...
            integer_sol[i] = intPart;
            fractPart -= epsilon;
            fractional_part[i] = fractPart;
            sol[i] -=epsilon;
            printf("So, x_%d = %lf + %lf\n", basv[i] + 1, fractional_part[i], intPart);
...
int main() // menu driven program
{
    ...
    ck = gomory(A, B, C, n, m, msum, sign, maxmin);
    //printf("%d",msum);
    return 0;
}